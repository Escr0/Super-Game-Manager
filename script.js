...
// TITRES HEADERS //
		...
		header1.innerHTML = 'TITRE DU JEU <div id=triAZ name="sortBy"id="sortBy">▲</div><div id=triZA name="sortBy"id="sortBy">▼</div>';
...
// POUR TRIER ON RAJOUT EVENT -> LORSQUE CLICK= DEMARRE LA FONCTION TRI A-Z
		...
		document.querySelector('#triAZ').addEventListener('click', (event) => {
				cm.listOfGames.sort(GameManager.compareByName);
			cm.displayGamesAsATable('games');
		})
//LORSQUE CLICK = DEMARRE LA FONCTION TRI INVERSE Z->A
		document.querySelector('#triZA').addEventListener('click', (event) => {
